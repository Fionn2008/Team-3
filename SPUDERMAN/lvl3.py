...
# Load Images
player1_img = pygame.image.load(join(BASE_DIR, "images", "spidy.png")).convert_alpha()
...
player1_shoot_img = pygame.image.load(join(BASE_DIR, "images", "spidy_shoot.png")).convert_alpha()
...
player2_img = pygame.image.load(join(BASE_DIR, "images", "docock.png")).convert_alpha()
...
# --- ENRAGED RED-TINTED DOC OCK ---
player2_angry_img = player2_img.copy()
red_overlay = pygame.Surface(player2_angry_img.get_size(), pygame.SRCALPHA)
red_overlay.fill((255, 60, 60, 255))  # Intense red tint
...
web_img_large = pygame.transform.scale(web_img, (200, 200))  # Mega Web
webs = []  
...
pygame.time.set_timer(SUPER_SPAWN_EVENT, 3000)  # Spawns every 3 seconds

# Particle Explosions list: [x, y, vx, vy, radius, color, life]
particles = []

bg_img = pygame.image.load(join(BASE_DIR, "images", "bridge.png")).convert()
...
def reset_game():  
    ...
    global is_shooting, invincible_timer, has_super_power, particles
    global p1_lives, p2_lives, game_over, winner_text, webs, health_items, super_items
    global is_shooting, invincible_timer, has_super_power, particles, last_shot_time
    p1_lives = 50  
    p2_lives = 100
    game_over = False  
    winner_text = ""  
...
def run_level():
    """
    Runs Level 3 (Doc Ock) to completion.
    Returns "win" once the player has beaten Doc Ock and pressed SPACE to
    continue. Only exits the process on window close (QUIT event).
    """
    global p1_lives, p2_lives, game_over, winner_text
    global has_super_power, is_shooting, shoot_timer, invincible_timer
    global last_shot_time, screen_shake
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            # Spawn Super Power Item (Golden Star Orb)
        ...
        if not game_over:
            ...
            ## 5. Doc Ock Melee Attack (Double Damage in Rage Mode)
            ...
            # 6. Web Movement, Collision & Super Explosion
            if is_shooting:
                shoot_timer -= 1
                if shoot_timer <= 0:
                    is_shooting = False
            ...
            for w in webs[:]:
                ...
                if w['rect'].colliderect(player2_rect):
                    ...
                    # Check for Super Web Impact
                    if w['is_super']:
                        screen_shake = 22  # Big Screen Shake
                        trigger_explosion(player2_rect.centerx, player2_rect.centery)  # Explosion!
            ...
            for p in particles[:]:
                p[0] += p[2]  # x += vx
                p[1] += p[3]  # y += vy
                p[4] = max(0, p[4] - 0.25)  # shrink radius
                p[6] -= 1  # life countdown
                if p[6] <= 0:
                    particles.remove(p)
        ...
        # Draw Super Power Collectible Orb (Golden pulsating sphere)
        ...
        # Draw Spider-Man (with aura if Super Powered)
        if has_super_power:
        ...
        spidey_lives_text = hud_font.render(f"Spidey HP: {p1_lives}/50", True, (0, 255, 100))
        screen.blit(spidey_lives_text, (20, 48))

        doc_lives_text = hud_font.render(f"Doc Ock HP: {p2_lives}/50", True, (255, 80, 80))
        ...
        if p2_lives <= 40:
            ...
            # Red warning text above his health bar
        ...
        if game_over:
            ...
            continue_msg = "Press SPACE to Continue" if won else "Press 'R' to Restart"
            restart_text = restart_font.render(continue_msg, True, (255, 215, 0))
