...
player_img = pygame.image.load(join("Shahreen folder/image/image/spidy.png")).convert_alpha()
rect = player_img.get_bounding_rect()
player_img = player_img.subsurface(rect).copy() 
player_img = scale_to_height(player_img, 100)
 
player_shoot_img = pygame.image.load(join("Shahreen folder/image/image/spidy_shoot.png")).convert_alpha()  
...
while True:
    ...
    if now - player_last_attack < 150:
        ...
        pygame.draw.line(screen, beam_color, player_rect.center, enemy_rect.center, beam_width)
    ...
    # Game Over Screen
    if game_over:
        winner = "THE GREEN GOBLIN" if player_health <= 0 else "SPIDER-MAN"
        text = font.render(f"{winner} Wins!", True, (255, 255, 255))
        text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
        pygame.draw.rect(screen, (0, 0, 0), text_rect.inflate(40, 30))
